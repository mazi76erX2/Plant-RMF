...
const Header = () => {
  ...
  return (
    <div className="relative">
      ...
      <div className={`w-full py-2 fixed top-0 left-0 z-50 transition-all ${
      ...
      {/* Main Navigation */}
      <motion.header 
        className={`w-full fixed ${scrolled ? 'top-0' : 'top-10'} left-0 z-40 transition-all duration-300 ${
          scrolled ? 'bg-white shadow-md py-2' : 'bg-black/30 py-4'
        } ${isMenuOpen ? 'bg-white' : ''}`}
        initial={{ y: 0, backgroundColor: scrolled ? 'rgba(255, 255, 255, 1)' : 'rgba(0, 0, 0, 0.3)' }}
        animate={{ 
          y: 0, 
          top: scrolled ? 0 : 40, // 40px is approximately the height of the top bar
          backgroundColor: scrolled || isMenuOpen ? 'rgba(255, 255, 255, 1)' : 'rgba(0, 0, 0, 0.3)',
          boxShadow: scrolled || isMenuOpen ? '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' : 'none'
        }}
          ...
          <div className="flex items-center justify-between">
              ...
              <div className="relative group">
                <Link 
                  ...
                  className={`transition-colors ${
                    scrolled ? 'text-gray-800 hover:text-green-700' : 'text-white hover:text-gray-200'
                  } font-medium`}
                >
                  About
                </Link>
            ...
            >
              <svg 
                ...
                className={`h-6 w-6 ${scrolled || isMenuOpen ? 'text-gray-800' : 'text-white'}`} 
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
              >
          ...
          <div className="lg:hidden bg-white shadow-lg py-4">
            <div className="container mx-auto px-4">
              <nav className="flex flex-col space-y-4">
                <Link href="/about" className="text-gray-800 hover:text-green-700 transition-colors block font-medium">
                  About
                </Link>
                <Link href="/trees-carbon" className="text-gray-800 hover:text-green-700 transition-colors block font-medium">
                  Trees & Carbon
                </Link>
                <Link href="/focus-areas" className="text-gray-800 hover:text-green-700 transition-colors block font-medium">
                  Focus Areas
                </Link>
                <Link href="/community-planning" className="text-gray-800 hover:text-green-700 transition-colors block font-medium">
                  Community Planning
                </Link>
                <Link href="/our-team" className="text-gray-800 hover:text-green-700 transition-colors block font-medium">
                  Our Team
                </Link>
                <Link href="/contact" className="text-gray-800 hover:text-green-700 transition-colors block font-medium">
                  Contact
                </Link>
                <a 
                  href="https://donate.trees.org.za/"
      ...
      <div className={`w-full ${scrolled ? 'h-16 md:h-20' : 'h-[4.5rem] md:h-[6.5rem]'} transition-all`}></div>
    </div>
  );
};
